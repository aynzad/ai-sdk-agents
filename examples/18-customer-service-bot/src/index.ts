...
function main() {
  console.log(chalk.bold.cyan("\n✈️  AI SDK Agents - 18-Customer Service Bot"));
  console.log(chalk.cyan("Agents: Triage → FAQ, Booking"));
  ...
  const ask = (): void => {
    rl.question(chalk.bold.white("You: "), (input) => {
      ...
      void (async () => {
        const result = await Runner.run(triageAgent, trimmed);
        ...
        if (handoffSteps.length > 0) {
          for (const step of handoffSteps) {
            ...
            console.log(chalk.dim(`  (routed: ${data.from} → ${data.to})`));
          }
        }
        console.log();
        ask();
